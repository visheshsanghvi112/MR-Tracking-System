...
export function Header({ className }: HeaderProps) {
  ...
  return (
    <header className={cn(
      ...
      <div className="container mx-auto flex h-16 sm:h-18 lg:h-20 items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Brand */}
        <div className="flex items-center gap-2 sm:gap-3 lg:gap-6 flex-shrink-0">
          <Logo linkTo="/dashboard" size="md" />
          
          {/* Desktop Navigation */}
          {!isMobile && <div className="ml-4 lg:ml-6"><NavigationMenuDesktop /></div>}
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 md:gap-3">
          ...
          <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted/30 border border-border/50">
            {wsConnected ? (
              ...
              <div className="flex items-center gap-2 text-destructive">
                ...
                <span className="text-xs font-medium whitespace-nowrap">Offline</span>
              </div>
            )}
          </div>
          ...
          <Button variant="ghost" size="sm" className="relative hidden sm:flex h-9 w-9 px-0 hover:bg-accent transition-colors rounded-full">
            <Bell className="h-4 w-4" />
            <Badge className="absolute -top-1 -right-1 h-5 w-5 rounded-full p-0 text-xs gradient-primary text-primary-foreground border border-background">
              3
            </Badge>
          </Button>
